...
버전 변경: 1.0.0 -> 1.1.0
수정된 원칙:
- I. Frontend-Backend Separation & Docs-as-Contract -> I. 프론트엔드-백엔드 분리와 문서 계약
- V. GitHub Traceability & Review Standardization -> V. GitHub 추적성과 리뷰 표준화
추가된 정책:
- 모든 프로젝트 문서는 한국어를 기본 언어로 사용
동기화가 필요한 템플릿:
...
### I. 프론트엔드-백엔드 분리와 문서 계약
React 프론트엔드와 Spring Boot 백엔드는 작업 경로, 책임, 배포 흐름을 분리해야 한다.
프론트엔드와 백엔드가 동시에 변경되는 기능은 구현보다 먼저 버전이 명시된 계약
문서를 `Docs/`에 기록해야 하며, API 변경은 해당 문서 업데이트 없이 머지될 수 없다.
...
이슈 생성이나 문서 없는 PR 병합은 허용되지 않는다. 또한 프로젝트에서 사람이 읽는
문서는 모두 한국어로 작성해야 하며, 문서 템플릿과 기존 산출물도 같은 기준을 따른다.
하나의 이슈에 대응하는 작업이 끝나면 해당 작업 범위를 기준으로 즉시 PR을 생성해야
하며, 여러 이슈를 하나의 PR에 불필요하게 합치는 방식은 지양한다.
리뷰 코멘트가 달린 경우 작성자는 각 코멘트에 답글을 남겨야 하며, 반영했으면 무엇을
반영했는지, 반영하지 않았으면 반영하지 않은 이유와 대안을 명시해야 한다.
이 원칙은 업무 단위를 추적 가능하게 유지하고, 리뷰 품질과 문서 일관성을 함께
보장하기 위한 것이다.
...
- 시나리오, 대사, 호감도 구간, 성격 타입, 렌더 포맷이 바뀌는 기능은 구현 착수 전에
  `Docs/dialogue-content-spec-vX.Y.md`와 `Backend/Reference/`의 기준 문서를 함께
  검토하고, 변경이 있으면 같은 변경 세트에서 동기화해야 한다.
...
- 저장소의 사람이 읽는 문서를 새로 만들거나 수정할 때는 한국어 표현을 기본으로 하고,
  식별자, 코드, 라벨 이름처럼 유지가 필요한 고유 표기는 필요한 범위에서만 사용한다.
...
**버전**: 1.2.0 | **비준일**: 2026-04-18 | **최종 개정일**: 2026-04-19
